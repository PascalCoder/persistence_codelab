...
public interface LoanDao {
    ...
    @Query("SELECT Book.title, User.name, Loan.endTime FROM Loan " +
            "INNER JOIN Book ON Loan.book_id = Book.id " +
            "INNER JOIN User ON Loan.user_id = User.id " +
            "WHERE User.name LIKE :userName " +
            "AND Loan.endTime > :date")
    LiveData<List<LoanWithUserAndBook>> findBooksBorrowedByUserAfter(String userName, Date date);
}
